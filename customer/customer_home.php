...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT TravelPackageID, Destination, DepartureDate, ReturnDate, FlightDetails, AccommodationDetails, Price FROM packages WHERE 1 $dateCondition $countryCondition";
    ...
    $sql = "SELECT TravelPackageID, Destination, DepartureDate, ReturnDate, FlightDetails, AccommodationDetails, Price FROM packages";
    $result = $connection->query($sql);
...
// Store the entire result set in the session
$_SESSION["TravelPackageResult"] = $result;
?>
...
echo '<button onclick="redirectToOrder(' . $row['TravelPackageID'] . ')">Select</button>';
echo '<script>console.log("Clicked on button with TravelPackageID: ' . $row['TravelPackageID'] . '")</script>';
            echo '</div>';
...
<script>
    function redirectToOrder(travelPackageID) {
        // Redirect to order.php with the TravelPackageID as a parameter
        window.location.href = 'order.php?TravelPackageID=' + travelPackageID;
    }
</script>
